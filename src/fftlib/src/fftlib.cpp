...
{	
	switch(*d)
		...
		case 2:
			{
				const data<FFTW, INT>::type_t nrev[2] = {n[1], n[0]};
				const data<FFTW, INT>::type_t inembedrev[2] = {inembed[1], inembed[0]};
				const data<FFTW, INT>::type_t onembedrev[2] = {onembed[1], onembed[0]};
				(*p) = fftw_plan_many_dft((*d), nrev, (*howmany), in, inembedrev, (*istride), (*idist), out, onembedrev, (*ostride), (*odist), (*sign), (*flags));
			}
			break;

		case 3:
			{
				const data<FFTW, INT>::type_t nrev[3] = {n[2], n[1], n[0]};
				const data<FFTW, INT>::type_t inembedrev[3] = {inembed[2], inembed[1], inembed[0]};
				const data<FFTW, INT>::type_t onembedrev[3] = {onembed[2], onembed[1], onembed[0]};
				(*p) = fftw_plan_many_dft((*d), nrev, (*howmany), in, inembedrev, (*istride), (*idist), out, onembedrev, (*ostride), (*odist), (*sign), (*flags));
			}
			break;
		...
		default:
			{
				FFTLIB_WARNING("dfftw_plan_many_dft_(thread=" << omp_get_thread_num() << ") only d=1,2,3 is currently supported");
			}
	    }
}
...
void dfftw_plan_many_dft_c2r_(trafo_base<FFTW>::plan_t* p, const data<FFTW, INT>::type_t* d, const data<FFTW, INT>::type_t* n, const data<FFTW, INT>::type_t* howmany, data<FFTW, C_64>::type_t* in, const data<FFTW, INT>::type_t* inembed, const data<FFTW, INT>::type_t* istride, const data<FFTW, INT>::type_t* idist, data<FFTW, R_64>::type_t* out, const data<FFTW, INT>::type_t* onembed, const data<FFTW, INT>::type_t* ostride, const data<FFTW, INT>::type_t* odist, const data<FFTW, UINT>::type_t* flags)
{
	switch(*d)
		...
		case 2:
			{
				...
				const data<FFTW, INT>::type_t inembedrev[2] = {inembed[1], inembed[0]};
				const data<FFTW, INT>::type_t onembedrev[2] = {onembed[1], onembed[0]};
				(*p) = fftw_plan_many_dft_c2r((*d), nrev, (*howmany), in, inembedrev, (*istride), (*idist), out, onembedrev, (*ostride), (*odist), (*flags));
			}
			break;
		...
		default:
			{
				FFTLIB_WARNING("dfftw_plan_many_dft_c2r_(thread=" << omp_get_thread_num() << ") only d=1,2,3 is currently supported");
			}
	    }
}
...
{	
	switch(*d)
		...
		case 2:
			{
				...
				const data<FFTW, INT>::type_t inembedrev[2] = {inembed[1], inembed[0]};
				const data<FFTW, INT>::type_t onembedrev[2] = {onembed[1], onembed[0]};
				(*p) = fftw_plan_many_dft_r2c((*d), nrev, (*howmany), in, inembedrev, (*istride), (*idist), out, onembedrev, (*ostride), (*odist), (*flags));
			}
			break;
		...
		default:
			{
				FFTLIB_WARNING("dfftw_plan_many_dft_r2c_(thread=" << omp_get_thread_num() << ") only d=1,2,3 is currently supported");
			}
	    }
}
